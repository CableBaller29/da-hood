...
Promo = {
    "🎭Monkey Ware On Top🎭",
    "🎭Get Monkey Ware🎭",
    "🎭Monkey Ware Best Lock🎭",
    "🎭Monkey Ware Best Da Hood Script🎭",
    "🎭Monkey WareUI On Git🎭",
    "🎭Monkey Ware Keyless🎭",
    "🎭Monkey Ware Best AutoStomp🎭",
    "🎭Monkey Ware Best Animations🎭"
}
    ...
    local function onKeyPress(input, gameProcessed)
        if not gameProcessed then
            if _G.autoStomp and input.KeyCode == stompKey then
                ...
                if isLooping then
                    showNotification("Auto Stomp Enabled", "🎭  Monkey Ware 🎭", 3)
                    startStomping()  -- Now calling startStomping properly
                else
                    showNotification("Auto Stomp Disabled", "🎭  Monkey Ware 🎭", 3)
                    stopStomping()
                end
            end
        end
    ...
    local function teleportAndStomp(targetPlayer)
        if targetPlayer and targetPlayer.Character then
            ...
            if targetHumanoid and torso and playerRoot then
                ...
                showNotification("Stomping", "🎭  Monkey Ware 🎭", 3)
            ...
            showNotification("No Target Found", "🎭  Monkey Ware 🎭", 3)
        end
    end
        ...
        elseif Config.enabled and (Config.spyOnMyself == true or p ~= player) then
            ...
            if hidden and Config.enabled then
                if Config.public then
                    saymsg:FireServer((Config.publicItalics and "/me " or '') .. "{Monkey Ware} [" .. p.Name .. "]: " .. msg, "All")
                else
...
WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('Monkey Ware V1 | %s fps | %s ms'):format(
        math.floor(FPS),
        ping
    ))
end)
